...
class Peer:
    ...
    def __init__(self, uID, speed, cpu, hashingFraction, genesisBlock, ttx, numPeers, selfish, stubborn, eventQueue, rho):
        ...
        self.eventQueue.put((miningTime, Event(0+miningTime, True, False, None, self, self.generateBlock())))        
        self.eventQueue.put((txTime, Event(0+txTime, False, False, None, self, None)))    
    ...
    def handleTransactionReceive(self, transaction: Transaction, time: float, sendPeer):
        ...
        for peer in self.connectedPeers:
            ...
            # latency
            latency = self.rho[self.uID][peer.uID] + transaction.size/c + d

            # adding event to queue
            self.eventQueue.put((time + latency, Event(time + latency, False, True, self, peer, transaction)))
    ...
    def handleTransactionGeneration(self, time: float):
        ...
        txTime = rd.exponential(scale=self.ttx, size=(1, 1))[0][0]
        self.eventQueue.put((time + txTime, Event(time + txTime, False, False, None, self, None)))
    ...
    def broadcastBlock(self, block: Block, time: int):
        ...
        for peer in self.connectedPeers:
            ...
            d = rd.exponential(scale=(96 * kbbits/c), size=(1, 1))[0][0]
            # latency
            latency = self.rho[self.uID][peer.uID] + block_length/c + d
            # adding event to queue
            self.eventQueue.put((time + latency, Event(time + latency, True, True, self, peer, block))) 
    ...
    def broadcast2Blocks(self, block1: Block, block2: Block, time: int):
        ...
        for peer in self.connectedPeers:
            ...
            d = rd.exponential(scale=(96 * kbbits/c), size=(1, 1))[0][0]
            # latency
            latency1 = self.rho[self.uID][peer.uID] + block1_length/c + d
            latency2 = self.rho[self.uID][peer.uID] + block2_length/c + d
            # adding event to queue
            self.eventQueue.put((time + latency1, Event(time + latency1, True, True, self, peer, block1))) 
            self.eventQueue.put((time + latency1 + latency2, Event(time + latency1 + latency2, True, True, self, peer, block2))) 
        ...
        if self.selfish or self.stubborn:
            if (self.lead == -1):
                ...
                if length > self.longestChainLength:
                    ...
                    self.eventQueue.put((time+miningTime, Event(time+miningTime, True, False, None, self, self.generateBlock()))) # add event to queue
    ...
    def handleBlockGeneration(self, block: Block, time: float):
        ...
        miningTime = rd.exponential(scale=interarrivalTime/self.hashingFraction, size=(1, 1))[0][0]
        # add event to queue
        self.eventQueue.put((time+miningTime, Event(time+miningTime, True, False, None, self, self.generateBlock())))   
...
def simulate(args):
    ...
    for i in range(args.peers):
        ...
        if i==0:
            peerList.append(Peer(i, speed, cpu, args.hf/100, genesisBlock, args.ttx, args.peers, False, True, eventQueue, rho))
        else:    
            peerList.append(Peer(i, speed, cpu, hashingFraction, genesisBlock, args.ttx, args.peers, False, False, eventQueue, rho))
    ...
    return [mpu_node_adv, mpu_node_overall, frac_att_main_chain]
...
if __name__ == '__main__':
    ...
    for hf in tqdm([10,20,30,40]):
        for tau in [75]:
            args_list = []
            for k in range(1000,1000+num_seeds):
                args.hf = hf
                args.tau = tau
                args.seed = k
                args_list.append(copy.deepcopy(args))
            with Pool() as pool:
                res = pool.starmap(simulate, zip(args_list))
            res = np.array(res)
